...
const authorize = (allowedRoles) => (req, res, next) => {
   ...
   if (!token) {
      return res.status(440).json({ message: 'Accès non autorisé. Token manquant.' });
   }
   ...
   try {
      ...
      if (!Object.values(roles).includes(decodedToken.role)) {
         return res.status(440).json({ message: 'Accès interdit. Rôle utilisateur incorrect.' });
      }
      ...
      return res.status(440).json({ message: 'Accès interdit. Token invalide.' });
   }
};
